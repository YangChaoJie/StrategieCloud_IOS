...
public class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
	...
	private var _needDefaultScale = false
    
    /// the color for the background of the chart-drawing area (everything behind the grid lines).
    public var gridBackgroundColor = NSUIColor(red: 240/255.0, green: 240/255.0, blue: 240/255.0, alpha: 1.0)
	
	public var borderColor = NSUIColor(red: 164/255.0, green: 170/255.0, blue: 179/255.0, alpha: 1.0)
	public var borderLineWidth: CGFloat = 0.8
	...
	public var longPressHighlighted = true
	...
	internal var _firstSetData: Bool = true
	internal var _restoreToDefaultScaleFactor = false
...
//        }
		...
		if self.longPressHighlighted {
			if (valuesToHighlight()) {
				renderer?.drawHighlighted(context: context, indices: _indicesToHighlight)
				renderer?.drawExternalData(context: context, withIndices: _indicesToHighlight, completionHandle: { [weak self] (datas: [ExternalData]) in
					if self!.delegate != nil {
						self!.delegate?.chartExternalDatas!(self!, datas: datas)
					}
					})
			} else {
				renderer?.drawExternalData(context: context, withIndices: [], completionHandle: { [weak self] (datas: [ExternalData]) in
					if self!.delegate != nil {
						self!.delegate?.chartExternalDatas!(self!, datas: datas)
					}
					})
	...
	public func restoreToDefaultScaleFactor() {
		_restoreToDefaultScaleFactor = true
	}

	private func scaleXToDefault() {
		func scale() {
			if _needDefaultScale {
				let scaleX_ = CGFloat(_data!.xValCount) * CGFloat(10) / CGFloat(400)
				let location = CGPoint(x: viewPortHandler.chartWidth - viewPortHandler.offsetRight, y: 0)
				self.zoom(scaleX_, scaleY: 1.0, x: location.x, y: location.y)
				self.setScaleMaximum(scaleX_, scaleY: 1.0)
			}
		}

		if _firstSetData && _data != nil {
			scale()
			_firstSetData = false
		}
		if _restoreToDefaultScaleFactor && _data != nil {
			if let _lastDataCount = lastDataCount {
				let newScaleX = CGFloat(_data!.xValCount) * _viewPortHandler.scaleX / _lastDataCount
				let maxScaleX = CGFloat(_data!.xValCount) * CGFloat(10) / CGFloat(400)
				let location = CGPoint(x: viewPortHandler.chartWidth - viewPortHandler.offsetRight, y: 0)
				self.zoom(newScaleX, scaleY: 1.0, x: location.x, y: location.y)
				self.setScaleMaximum(maxScaleX, scaleY: 1.0)
			}
			_restoreToDefaultScaleFactor = false
		}
	}
...
//            }
	...
	private func setHighlightToNil() {
		self.highlightValue(highlight: nil, callDelegate: true)
		self.lastHighlighted = nil
	}
	...
	@objc private func longPressGestureRecognized(recognizer: NSUILongPressGestureRecognizer) {
		...
		if recognizer.state == NSUIGestureRecognizerState.Began ||
			...
			let h = getHighlightByTouchPoint1(recognizer.locationInView(self))
		...
		} else if recognizer.state == NSUIGestureRecognizerState.Cancelled ||
			recognizer.state == NSUIGestureRecognizerState.Ended ||
			recognizer.state == NSUIGestureRecognizerState.Failed {
//			setHighlightToNil()
		}
	}
...
//                    }
            ...
            if (_isScaling)
            {
                ...
                if canZoomMoreX || canZoomMoreY
                {
                    ...
                    if (delegate !== nil)
                    {
						...
						delegate?.chartScaled?(self, scaleX: scaleX, scaleY: scaleY, dX: location.x, dY: location.y)
                    }
                }
    ...
    private func performPanChange(var translation translation: CGPoint) -> Bool
    {
        ...
        if (delegate !== nil)
        {
			...
			delegate?.chartMatrixTranslate!(self, transform: matrix)
        }
	...
	public func translated(transform: CGAffineTransform) {
		_viewPortHandler.refresh(newMatrix: transform, chart: self, invalidate: true)
	}
	...
	public func setScaleMaximum(scaleX: CGFloat, scaleY: CGFloat) {
		_viewPortHandler.setMaximumScaleX(scaleX)
		_viewPortHandler.setMaximumScaleY(scaleY)
	}
	...
	public var needScaleToDefault: Bool {
		set { _needDefaultScale = newValue }
		get { return _needDefaultScale }
	}
...
//    public var highlightPerDragEnabled = true
	public var highlightPerDragEnabled = false
	...
	public func getHighlightByTouchPoint1(pt: CGPoint) -> ChartHighlight? {
	
		if _data === nil {
			Swift.print("Can't select by touch. No data set.")
			return nil
		}
		return self.highlighter?.getHighlight(x: Double(pt.x))
	}
