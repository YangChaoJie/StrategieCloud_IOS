...
public protocol ChartViewDelegate
{
	...
	optional func chartScaled(chartView: ChartViewBase, scaleX: CGFloat, scaleY: CGFloat, dX: CGFloat, dY: CGFloat)
    
    // Callbacks when the chart is moved / translated via drag gesture.
    optional func chartTranslated(chartView: ChartViewBase, dX: CGFloat, dY: CGFloat)
	
	optional func chartMatrixTranslate(chartView: ChartViewBase, transform: CGAffineTransform)

	
	optional func chartExternalDatas(chartView: ChartViewBase, datas: [ExternalData])
	
	optional func chartTap(chartView: ChartViewBase)
}
...
//    public var noDataText = "No chart data available."
	...
	private let _activityIndicatorViewSize = CGSize(width: 80, height: 80)
	public var activityIndicatorView: UIActivityIndicatorView!
	
	public var dataChanged = false
	
	public var lastDataCount: CGFloat?
    ...
    internal func initialize()
    {
		...
		activityIndicatorView.frame.origin = CGPointMake(
			(self.bounds.size.width - _activityIndicatorViewSize.width)/2,
			(self.bounds.size.height - _activityIndicatorViewSize.height)/2)
		activityIndicatorView.frame.size = _activityIndicatorViewSize
		activityIndicatorView.hidesWhenStopped = true
		self.addSubview(activityIndicatorView)
    }
	
	public func setActivityViewHide(hide hide: Bool) {
		...
		if hide {
			...
			if !activityIndicatorView.isAnimating() {
				...
				GCDTimer.delay(20, block: {
					self.setActivityViewHide(hide: true)
				})
			}
		}
	}
    ...
    public var data: ChartData?
    {
        ...
        set
        {
			self.dataChanged = (_data != newValue)
            _offsetsCalculated = false
			if let data = _data {
				lastDataCount = CGFloat(data.xValCount)
			}
			_data = newValue
    ...
    public override func drawRect(rect: CGRect)
    {
        ...
        if _data === nil
        {
			...
			setActivityViewHide(hide: false)
