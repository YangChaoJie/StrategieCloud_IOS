        ...
        { // only one color per dataset
            ...
            if e1 != nil
            {
                ...
                for (var x = count > 1 ? minx + 1 : minx, j = 0; x < count; x++)
                {
					...
					if let validData = e1.data as? Bool {
						if !validData { continue }
					}
					
					if let validData = e2.data as? Bool {
						if !validData { continue }
					}
...
//                let size = max((count - minx - 1) * 2, 2)
	...
	private func parseExternalData(context context: CGContext, dataSet: ILineChartDataSet, index: Int) -> [ExternalData]? {
		let entryCount = dataSet.entryCount
		if entryCount > 0 {
			var externalDatas = [ExternalData]()
			if let e = dataSet.entryForXIndex(index) {
				if let data = e.data as? [String] {
					let change = (data[0] as NSString).doubleValue
					let reviseChange = NSString(format: "%.2f", change) as String
					let color = change >= 0 ? UIColor.KLineUpColor() : UIColor.KLineDownColor()
					externalDatas.append(ExternalData(name: "涨幅", value: reviseChange+"%", color: color))
				}
			}
			
			if let name = dataSet.label {
				let value = NSString(format: "%.2f", dataSet.yValForXIndex(index)) as String
				externalDatas.append(ExternalData(name: name, value: value, color: dataSet.colorAt(0)))
			}
		...
		for i in 0 ..< lineData.dataSetCount {
			...
			if set.isVisible {
				...
				if indices.count > 0 {
					...
					let lineSet = set as! LineChartDataSet
					if let maxIndex = lineSet.maxIndex {
						selectedIndex = maxIndex
					}
				}
				
				if let data = self.parseExternalData(context: context, dataSet: set as! ILineChartDataSet, index: selectedIndex) {
					datas += data
				}
			}
		}
    ...
    public override func drawHighlighted(context context: CGContext, indices: [ChartHighlight])
    {
        ...
        for (var i = 0; i < indices.count; i++)
        {
			...
			//			let point = CGPoint(x: viewPortHandler.contentLeft, y: _highlightPointBuffer.y)
			//			ChartUtils.drawText(context: context, text: "sdf;sldf", point: point, align: NSTextAlignment.Center, attributes: [NSForegroundColorAttributeName: UIColor.redColor(), NSBackgroundColorAttributeName: UIColor.blackColor()])
        }
