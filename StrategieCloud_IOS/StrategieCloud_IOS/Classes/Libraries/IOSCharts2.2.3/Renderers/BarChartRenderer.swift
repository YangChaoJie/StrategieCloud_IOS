...
public class BarChartRenderer: ChartDataRendererBase
{
    ...
    public func drawDataSet(context context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        for (var j = 0, count = Int(ceil(CGFloat(dataSet.entryCount) * animator.phaseX)); j < count; j++)
        {
            ...
            if (!containsStacks || vals == nil)
            {
				...
				var color: CGColor = dataSet.colorAt(j).CGColor
				if let increasingColor = dataSet.increasingColor, descreasingColor = dataSet.decreasingColor {
					if let fill = e.data as? Bool {
						color = fill ? descreasingColor.CGColor : increasingColor.CGColor
					} else {
						color = (y >= 0) ? increasingColor.CGColor : descreasingColor.CGColor
					}
				}
	...
	private func parseExternalData(dataSet set: IBarChartDataSet, index: Int) -> ExternalData? {
		let entryCount = set.entryCount
		if entryCount > 0 {
			if let name = set.label {
				let yVal = set.yValForXIndex(index)
				if yVal >= 0 {
					let value = NSString(format: "%.2f", set.yValForXIndex(index)) as String
					if let e = set.entryForIndex(index) as? BarChartDataEntry {
						var color: UIColor = set.colorAt(index)
						if let increasingColor = set.increasingColor, decreasingColor = set.decreasingColor {
							if let fill = e.data as? Bool {
								color = fill ? decreasingColor : increasingColor
							} else {
								color = (e.value >= 0) ? increasingColor : decreasingColor
							}
						}
						if name == "VOL" {
							let reviseValue = Converts.unitConvert(value)
							return ExternalData(name: "量", value: reviseValue + "手", color: color)
						} else {
							return ExternalData(name: name, value: value, color: color)
						}
					}
				} else {
					return ExternalData(name: "量", value: "--", color: UIColor.TextHintColor())
				}
			}
		}
		return nil
	...
	public override func drawExternalData(context context: CGContext, withIndices indices: [ChartHighlight], completionHandle: ([ExternalData]) -> Void) {
		...
		for set in barData.dataSets as! [IBarChartDataSet] {
			if set.isVisible && set.entryCount > 0 {
				...
				if let data = self.parseExternalData(dataSet: set, index: selectedIndex) {
					datas.append(data)
				}
			}
		}
...
//            CGContextSetAlpha(context, set.highlightAlpha)
            ...
            if (CGFloat(index) < (CGFloat(dataProvider.chartXMax) * animator.phaseX) / CGFloat(setCount))
            {
				...
				let y = CGFloat(yValue) * animator.phaseY
				...
				if let _data = dataProvider.data {
					if _data.xVals.count > xIndex {
						if let time = _data.xVals[xIndex] {
							let attrs = [
								NSForegroundColorAttributeName: UIColor.whiteColor(),
								NSBackgroundColorAttributeName: UIColor.blackColor(),
								NSFontAttributeName: UIFont.systemFontOfSize(10)
							]
							let textWidth = time.sizeWithAttributes(attrs).width
							var point = CGPoint(x: _highlightPointBuffer.x, y: viewPortHandler.contentBottom + 2)
							if (_highlightPointBuffer.x - textWidth/2 < viewPortHandler.contentLeft) {
								point.x = viewPortHandler.contentLeft + textWidth / 2
							} else if (_highlightPointBuffer.x + textWidth / 2 > viewPortHandler.contentRight) {
								point.x = viewPortHandler.contentRight - textWidth / 2
							}
							ChartUtils.drawText(context: context, text: time, point: point, align: NSTextAlignment.Center, attributes: attrs)
						}
